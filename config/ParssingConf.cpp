...
LocationConfig ParssingConf::parseLocationBlock(const std::vector<std::string> &tokens, size_t &i, const std::string &path)
{
    LocationConfig loc;
    loc.setPath(path);
...
void ParssingConf::validate()
{
    for (size_t i = 0; i < _configs.size(); ++i)
    {
        for (size_t j = i + 1; j < _configs.size(); ++j)
        {
            ...
            {
                error("Duplicate server block: port=" + intToStr(_configs[i].getPort()) + " server_name=" + _configs[i].getServerName());
            }
        }
    }
}
...
const std::vector<Config> &ParssingConf::getConfigs() const
{
    return _configs;
}
...
void ParssingConf::parseConfig(const std::string &filename)
{
    ...
    while (i < tokens.size())
    {
        if (tokens[i] == "server")
        {
            ++i;
            if (i >= tokens.size() || tokens[i] != "{")
                error("Expected '{' after 'server', got '" + (i < tokens.size() ? tokens[i] : "EOF") + "'");
            ++i;
